...
const ProfileStats = () => {
	...
	useEffect(() => {
		...
		const hitsGraphData = mockData.map((item) => ({
			x: item.title,
			y: item.hits,
			label: "Hits",
		}));
		const commentsGraphData = mockData.map((item) => ({
			x: item.title,
			y: item.comments,
			label: "Comments",
		}));
		const likesGraphData = mockData.map((item) => ({
			x: item.title,
			y: item.likes,
			label: "Likes",
		}));

		const pieGraphData = mockData.map((item) => ({
			x: item.title,
			y: item.hits,
			label: item.title,
		}));
	...
	return (
		<section className="p-4 sm:px-0">
			...
			<article className="my-4 grid grid-cols-1 gap-6 lg:grid-cols-2">
				<div className="font-heading col-span-full flex justify-around rounded-lg p-4 font-medium shadow-lg">
					<p>
						Total Hits:{" "}
				...
				<div className="font-heading col-auto rounded-lg p-4 font-medium shadow-lg">
					<VictoryPie
						data={pieChartData}
