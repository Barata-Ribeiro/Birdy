...
export function attemptToGetUserIdFromToken(
    token: string,
    key: string
): string {
    try {
        ...
        if (!isUUIDValid(id))
            throw new UnauthorizedError("Invalid ID in token.")
...
export async function saveEntityToDatabase<T extends ObjectLiteral>(
    ...
    try {
        ...
        throw new InternalServerError(
            `An error occurred while saving the entity ${entity}.`
        )
    }
}
