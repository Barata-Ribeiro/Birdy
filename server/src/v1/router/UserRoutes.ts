...
routes.get("/profile/:username", (req, res, next) => userController.getUserProfile(req, res).catch(next))

routes.get("/profile/:username/follows", (req, res, next) => followsController.getAllUserFollows(req, res).catch(next))

routes.get("/profile/:username/followed-by", (req, res, next) =>
    followsController.checkIfUserIsFollowed(req, res).catch(next)
)

routes.get("/me/context", authMiddleware, (req, res, next) => userController.getUserContext(req, res).catch(next))

routes.get("/me/:userId", authMiddleware, (req, res, next) => userController.getPrivateProfile(req, res).catch(next))
...
routes.put("/me/:userId", authMiddleware, (req, res, next) => userController.updatePrivateProfile(req, res).catch(next))
